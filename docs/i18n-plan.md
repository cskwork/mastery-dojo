...
> 현재 상태: **완료 (2026-06-06).** `npm run check`(tsc + vitest + next build) 통과. 12개 도메인 전체가 한국어 오버레이를 가지며, 드릴 단위(prompt/hint/explanation)까지 한국어로 렌더된다. 회귀 가드: `test/i18nCoverage.test.ts`가 모든 드릴의 prompt/hint/explanation에 한글이 포함됨을 강제한다(코드 토큰·명령어·공식 명칭은 언어 중립으로 한국어 문장 안에 보존). 아래 "남은 작업"은 모두 처리됨.
